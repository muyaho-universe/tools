...
def _prepare_build(profile: BuildProfile, env: dict[str, str]) -> tuple[bool, str]:
    ...
    if configure_missing:
        ...
        if (not configure_path.exists()) and profile.name == "dwg2dxf":
            ok, err = _patch_dwg2dxf_configure_ac(profile.repo_dir)
            if not ok:
                return False, err
            autogen_err = ""
            autogen = profile.repo_dir / "autogen.sh"
            if autogen.exists():
                ok, autogen_err = run_cmd(["sh", "./autogen.sh"], cwd=profile.repo_dir, env=env)
            else:
                ok = False
                autogen_err = "missing autogen.sh"
            if (not ok) and ((profile.repo_dir / "configure.ac").exists() or (profile.repo_dir / "configure.in").exists()):
                ok, err = run_cmd(["autoreconf", "-fi"], cwd=profile.repo_dir, env=env)
                if not ok and (not configure_path.exists()):
                    return False, f"{autogen_err}\n{err}"
            if not configure_path.exists():
                return False, autogen_err or "dwg2dxf configure script was not generated"
            run_cmd(["chmod", "+x", str(configure_path)], cwd=profile.repo_dir, env=env)
